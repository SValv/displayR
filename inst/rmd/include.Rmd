...
Since i am farely new to coding and have no expirience with development of anything this is actually very likely.
I'd also welcome feedback of any kind to improve this little baby-project. 

This project is generally directed to people with little expirience in R who are interessted in easy and quick visualisation and exploration.

### Usage 

displayR has only one function called "displayR" it has five arguments:

DF(default="None"): a dataframe or object convertable to a dataframe that will be displayed. If you dont specify a dataframe, a menu to select one will open. It works with most formats like .txt, .csv etc (using fread). It cant open .xlsx files, if you encounter any problems with loading the data, try loading it manually.
```{r, eval=F}
displayR()
displayR(data)
```
...
limit(default=*11*): describes the maximum number of instances where the variable is converted into a factor, if the instances(determined by the length of the unique-vector) are higher, the variable wont be converted.
```{r, eval=F}
displayR(data,factordeclare = TRUE,limit = 8)
```

na.rm(default= *TRUE*): this removes missing values from the dataframe, it is recommended to be set to TRUE, because many functionalities of displayR wont work with missing values.
```{r, eval=F}
displayR(data,na.rm = FALSE)
```
...
```{}
c("blue","red","green","orange","violet")
```
...
Thanks a lot to the people providing these packages and to my colleage and friend Lena Pfeiffer who helped a lot with creative input.
